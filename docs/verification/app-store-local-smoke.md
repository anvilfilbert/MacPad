...
| Full Keyboard Access focus order | `FAIL` | Full Keyboard Access was temporarily enabled and verified on, then restored to off after the test. Save As traversed name, tags, location, disclosure, encoding, Cancel, and Save coherently. Find traversed term, Match Case, Wrap Around, Next, Previous, and back to term. Go To Line traversed line field, Cancel, and Go To. The About panel trapped keyboard focus in its read-only contact text area: Tab, Shift-Tab, and Control-Tab did not reach the Website, Support, or Privacy links even though Accessibility exposed those links. This exact-runtime failure requires a corrected candidate and owner retest; the successful dialog observations remain valid narrow evidence. |
